...
##    print(s)
    ...
    if data[2] != hash_value:
        cursor.execute("UPDATE files SET hash='"+hash_value+"' WHERE file='"+file+"' AND directory='"+path+"'")
        cursor.execute("UPDATE files SET action='"+action+"' WHERE file='"+file+"' AND directory='"+path+"'")
...
##    print(s)
    log.write(s+'\n')
    cursor.execute("INSERT INTO files(file,directory,hash,action,size) VALUES('"+file+"','"+path+"','"+hash_value+"','"+action+"','"+size+"')")
    connexion.commit()
